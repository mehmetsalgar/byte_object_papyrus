...
public class MyBenchmarkOrders extends AbstractBenchmark {
    ...
    private static final int NUMBER_OF_ITERATIONS = 7000;
    ...
    public static class MySetupOrders {
        ...
        public void initialize() {
            orders = new Orders();
            Order[] order = new Order[NUMBER_OF_ITERATIONS];
            for (int i = 0; i < NUMBER_OF_ITERATIONS; i++) {
                order[i] = createOrder(i);
            }
    ...
    public void testMethod(MySetupOrders setup) {
        ...
        //log.info("length: " + serialize(setup.orders));
        serialize(setup.orders);
    }

    private int serialize(Orders orders) {
        Orders serializedOrders = null;

        try {

            org.salgar.byte_object.io.FastByteArrayOutputStream bos = new org.salgar.byte_object.io.FastByteArrayOutputStream(2048);
            byte[] buff = new byte[2048];
            orders.marshal(bos, buff);
...
//            serializedOrder = unmarshaller.next();
            ...
            serializedOrders = new Orders();
            serializedOrders.unmarshal(serialized, 0);

            Order serializedOrder = serializedOrders.getOrder()[75];
    ...
    private int serialize(Orders orders) {
        ...
        try {
            ...
            org.salgar.byte_object.io.FastByteArrayOutputStream bos = new org.salgar.byte_object.io.FastByteArrayOutputStream();
            byte[] buff = new byte[2048];
            orders.marshal(bos, buff);
